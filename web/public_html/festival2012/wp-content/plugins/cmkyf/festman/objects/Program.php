...
class Program extends PersistentObject
{
	...
	public static function getProgram($program_id)
	{
		...
		$query_string = "SELECT * from " . $fm_db->getProgramTableName() . " WHERE Program_Id=".fmQueryValue($program_id);
	...
	public static function deleteProgram($program_id)
	{
		...
		$quety_string = "UPDATE " . $fm_db->getEventTableName() . " SET Program_Id=NULL WHERE Program_Id=".fmQueryValue($program_id);
		mysql_query($query_string);
		$quety_string = "DELETE FROM " . $fm_db->getProgramTableName() . " WHERE Program_Id=".fmQueryValue($program_id);
		mysql_query($query_string);
	}
	...
	protected function fillProgram_ProgramItems()
	{
		...
		{
			$query_string = "SELECT * FROM " . $fm_db->getProgram_ProgramItemTableName() . " WHERE Program_Id=" . fmQueryValue($this->Program_Id) . " ORDER BY Position";
			$result = mysql_query($query_string);
			$this->Program_ProgramItems = array();
...
function createOrUpdateProgram(Program $program)
{
	...
	$id          = fmQueryValue($program->getId());
	$name        = fmQueryValue($program->getName());
	$description = fmQueryValue($program->getDescription());

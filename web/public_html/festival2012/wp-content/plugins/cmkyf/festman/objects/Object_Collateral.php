...
class Object_Collateral extends PersistentObject
{
	...
	public static function getCollateralCount($object_name, $object_id)
	{
		...
		$query_string = "SELECT COUNT(*) FROM " . $fm_db->getObject_CollateralTableName() . " WHERE ObjectTable_Id=" . fmQueryValue($object_table->getId()) . " AND Object_Id=" . fmQueryValue($object_id);
	...
	public static function getCollateralInstanceCount($object_name, $collateral_id)
	{
		...
		$object_table = ObjectTable::getObjectTableByClassName($object_name);
		$query_string = "SELECT COUNT(*) FROM " . $fm_db->getObject_CollateralTableName() . " WHERE ObjectTable_Id=".fmQueryValue($object_table->getId())." AND Collateral_Id=" . fmQueryValue($collateral_id);
		$result = mysql_query($query_string);
	...
	public static function getAllCollateralInstanceCount($collateral_id)
	{
		...
		$query_string = "SELECT COUNT(*) FROM " . $fm_db->getObject_CollateralTableName() . " WHERE Collateral_Id=" . fmQueryValue($collateral_id);
		$result = mysql_query($query_string);
	...
	public static function deleteObject_Collateral($object_collateral_id)
	{
		...
		$query_string = "DELETE from " . $fm_db->getObject_CollateralTableName() . " WHERE Object_Collateral_Id=".fmQueryValue($object_collateral_id);
	...
	public static function deleteAllObject_Collateral(ObjectTable $object_table, $object_id)
	{
		...
		$query_string = "DELETE from " . $fm_db->getObject_CollateralTableName() . " WHERE ObjectTable_Id=".fmQueryValue($object_table->getId())." AND Object_Id=".fmQueryValue($object_id);
		mysql_query($query_string);
	}
...
function createOrUpdateObject_Collateral(Object_Collateral $object_collateral)
{
	...
	$id                      = fmQueryValue($object_collateral->getId());
	$object_table_id         = fmQueryValue($object_collateral->getObjectTable_Id());
	$object_id               = fmQueryValue($object_collateral->getObject_Id());
	$collateral_id           = fmQueryValue($object_collateral->getCollateral_Id());
	$sort_order              = fmQueryValue($object_collateral->getSortOrder());
	$is_default              = fmQueryValue($object_collateral->getIsDefault());

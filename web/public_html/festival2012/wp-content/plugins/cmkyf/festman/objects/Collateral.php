...
class Collateral extends PersistentObject
{
	...
	public static function getCollateralByCollateralLocation($collateral_location_name)
	{
		...
		{
			$query_string = "SELECT * from " . $fm_db->getCollateralTableName() . " WHERE Location IN (SELECT Location FROM CollateralLocation WHERE Name='".fmQueryValue($collateral_location_name)."')";
		}

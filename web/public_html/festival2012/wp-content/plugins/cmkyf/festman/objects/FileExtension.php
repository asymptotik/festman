...
class FileExtension extends PersistentObject
{
	...
	{
		global $fm_db;
		$query_string = "SELECT * FROM " . $fm_db->getFileExtensionTableName() . " WHERE FileExtension_Id=".fmQueryValue($file_extension_id);
	...
	{
		global $fm_db;
		$query_string = "SELECT * FROM " . $fm_db->getFileExtensionTableName() . " WHERE Extension=".fmGetQuotedStringOrNull(fmQueryValue($extension));
	...
	{
		global $fm_db;
		$query_string = "DELETE from " . $fm_db->getFileExtensionTableName() . " WHERE FileExtension_Id=".fmQueryValue($file_extension_id);
...
function createOrUpdateFileExtension(FileExtension $file_extension)
{
	...
	$extension    = fmQueryValue($collateral->getExtension());
	$mime_type_id = fmQueryValue($collateral->getMimeType());

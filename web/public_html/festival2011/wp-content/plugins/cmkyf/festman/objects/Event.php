...
class Event extends AbstractCollateralCollection
{
	...
	{
		global $fm_db;
		$query_string = "SELECT * from " . $fm_db->getEventTableName() . " WHERE Event_Id=".queryValue($event_id);
	...
	public static function deleteEvent($event_id)
	{
		...
		$query_string = "DELETE FROM " . $fm_db->getProgram_ProgramItemTableName() . " WHERE Program_Id IN (SELECT Program_Id FROM Event WHERE Event_Id=".queryValue($event_id).")";
		mysql_query($query_string);
		$query_string = "DELETE FROM " . $fm_db->getProgramTableName() . " WHERE Program_Id IN (SELECT Program_Id FROM Event WHERE Event_Id=".queryValue($event_id).")";
		mysql_query($query_string);
		$query_string = "DELETE FROM " . $fm_db->getEventTableName() . " WHERE Event_Id=".queryValue($event_id);
		mysql_query($query_string);
	}
...
function createOrUpdateEvent(Event $event)
{
	...
	$id          = queryValue($event->getId());
	$name        = queryValue($event->getName());
	$location_id = queryValue($event->getLocation_Id());
	$description = queryValue($event->getDescription());
	$start_time  = queryValue($event->getStartTime());
	$end_time    = queryValue($event->getEndTime());
	$program_id  = queryValue($event->getProgram_Id());

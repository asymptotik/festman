...
class Collateral extends PersistentObject
{
	...
	public static function getCollateral($collateral_id)
	{
		...
		$query_string = "SELECT * from " . $fm_db->getCollateralTableName() . " WHERE Collateral_Id=".queryValue($collateral_id);
	...
	{
		global $fm_db;
		$query_string = "SELECT * from " . $fm_db->getCollateralTableName() . " WHERE MedaiType=".queryValue($media_type);
		$result = mysql_query($query_string);
        $ret = array();
	...
	public static function getCollateralByCollateralLocation($collateral_location_name)
	{
		...
		{
			$query_string = "SELECT * from " . $fm_db->getCollateralTableName() . " WHERE Location IN (SELECT Location FROM CollateralLocation WHERE Name='".queryValue($collateral_location_name)."')";
		}
	...
	{
		global $fm_db;
		$query_string = "SELECT Url FROM " . $fm_db->getCollateralTableName() . " WHERE Collateral_Id=".queryValue($collateral_id);
		$result = mysql_query($query_string);
		$collateral_url = "";
...
function createOrUpdateCollateral(Collateral $collateral)
{
	...
	$mime_type_id = queryValue($collateral->getMimeType_Id());
	$caption      = queryValue($collateral->getCaption());
	$location     = queryValue($collateral->getLocation());
	$name         = queryValue($collateral->getName());

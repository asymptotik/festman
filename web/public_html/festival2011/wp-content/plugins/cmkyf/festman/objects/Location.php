...
class Location extends AbstractCollateralCollection
{
	...
	public static function getLocation($location_id)
	{
		...
		$query_string = "SELECT * from " . $fm_db->getLocationTableName() . " WHERE Location_Id=".queryValue($location_id);
	...
	public static function deleteLocation($location_id)
	{
		...
		$query_string = "DELETE from " . $fm_db->getLocationTableName() . " WHERE Location_Id=".queryValue($location_id);
		mysql_query($query_string);
		$quety_string = "UPDATE " . $fm_db->getEventTableName() . " SET Location_Id=NULL WHERE Location_Id=".queryValue($location_id);
		mysql_query($query_string);
	}
	...
	public function getAllEvents()
	{
		...
		$query_string = "SELECT * from " . $fm_db->getEventTableName() . " WHERE Location_Id = ".queryValue($this->getId()) . " ORDER BY StartTime";
		$result = mysql_query($query_string);
		$ret = array();
...
function createOrUpdateLocation(Location $location)
{
	...
	$id           = queryValue($location->getId());
	$name         = queryValue($location->getName());
	$url          = queryValue($location->getUrl());
	$url_text     = queryValue($location->getUrlText());
	$map_url      = queryValue($location->getMapUrl());
	$map_url_text = queryValue($location->getMapUrlText());
	$address      = queryValue($location->getAddress());
	$city         = queryValue($location->getCity());
	$state        = queryValue($location->getState());
	$zip_code     = queryValue($location->getZipCode());
	$description  = queryValue($location->getDescription());

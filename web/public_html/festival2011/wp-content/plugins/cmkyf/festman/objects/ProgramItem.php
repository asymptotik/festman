...
class ProgramItem extends AbstractCollateralCollection
{
	...
	public static function getTypedProgramItem($program_item_id, $class)
	{
		...
		$query_string = "SELECT * from " . $fm_db->getProgramItemTableName() . " WHERE ObjectClass=\"".queryValue($class)."\" AND ProgramItem_Id=".queryValue($program_item_id);
	...
	{
		global $fm_db;
		$query_string = "SELECT * FROM " . $fm_db->getProgramItemTableName() . " WHERE ObjectClass=\"".queryValue($class)."\" ORDER BY Name";
		$result = mysql_query($query_string);
		$ret = array();
	...
	{
		global $fm_db;
		$query_string = "SELECT ObjectClass from " . $fm_db->getProgramItemTableName() . " WHERE ProgramItem_Id=".queryValue($program_item_id);
	...
	public static function getProgramItem($program_item_id)
	{
		...
		$query_string = "SELECT * from " . $fm_db->getProgramItemTableName() . " WHERE ProgramItem_Id=\"".queryValue($program_item_id)."\"";
		$result = mysql_query($query_string);
	...
	public static function deleteProgramItem($program_item_id)
	{
		...
		$query_string = "DELETE from " . $fm_db->getProgramItemTableName() . " WHERE ProgramItem_Id=".queryValue($program_item_id);
	...
	public static function deleteProgramItemRelatedPersons($program_item_id)
	{
		...
		$query_string = "SELECT RelatedPerson_Id FROM " . $fm_db->getRelatedPersonTableName() . " WHERE ProgramItem_Id=".queryValue($program_item_id);
	...
	public static function getProgramItemSelectOptions($class, $selected_program_item_id)
	{
		...
		$query_string = "SELECT Name, ProgramItem_Id from " . $fm_db->getProgramItemTableName() . " WHERE ObjectClass=\"".queryValue($class)."\"";
		$result = mysql_query($query_string);
		$ret = "";
	...
	public function removeRelatedPersonById($related_person_id)
	{
	    for($i = 0; $i < count($this->RelatedPersons); $i++)
	    {
	        if($this->RelatedPersons[$i]->RelatedPerson_Id == $related_person_id)
	        {
	...
	protected function fillRelatedPersons()
	{
		...
		{
			$query_string = "SELECT * FROM " . $fm_db->getRelatedPersonTableName() . " WHERE ProgramItem_Id=" . queryValue($this->ProgramItem_Id);
			$result = mysql_query($query_string);
...
function createOrUpdateProgramItem(ProgramItem $program_item)
{
	...
	$id           = queryValue($program_item->getId());
	$name         = queryValue($program_item->getName());
	$url          = queryValue($program_item->getUrl());
	$url_text     = queryValue($program_item->getUrlText());
	$origin       = queryValue($program_item->getOrigin());
	$object_class = queryValue($program_item->getObjectClass());
	$description  = queryValue($program_item->getDescription());

...
class Program_ProgramItem extends PersistentObject
{
	...
	public static function deleteProgram_ProgramItem($ppi_id)
	{
		...
		$query_string = "DELETE from " . $fm_db->getProgram_ProgramItemTableName() . " WHERE Program_ProgramItem_Id=".queryValue($ppi_id);
	...
	public static function deleteProgram_ProgramItemByProgramItemId($program_item_id)
	{
		...
		$query_string = "DELETE from " . $fm_db->getProgram_ProgramItemTableName() . " WHERE ProgramItem_Id=".queryValue($program_item_id);
		mysql_query($query_string);
	}
	...
	public function validate()
	{
		...
		$error_message = appendLine($error_message, $start_time_validator->validate($this->StartTime));
		
		$program_id_validator = new Validator();
		$program_id_validator->addValidator(new RequiredValidator("A Program is required."));
		$error_message = appendLine($error_message, $program_id_validator->validate($this->Program_Id));
		
		$program_item_validator = new Validator();
		$program_item_validator->addValidator(new RequiredValidator("A Program Item is required."));
		$error_message = appendLine($error_message, $program_item_validator->validate($this->ProgramItem_Id));
...
function createOrUpdateProgram_ProgramItem(Program_ProgramItem $ppi)
{
	...
	$id              = queryValue($ppi->getId());
	$position        = queryValue($ppi->getPosition());
	$start_time      = queryValue($ppi->getStartTime());
	$program_id      = queryValue($ppi->getProgram_Id());
	$program_item_id = queryValue($ppi->getProgramItem_Id());

	if($id != NULL)
	{
		// update
		$query_string = "UPDATE " . $fm_db->getProgram_ProgramItemTableName() . " Set" .
		" Position=".getQuotedStringOrNull($position).
		",StartTime=".getQuotedStringOrNull($start_time).
	...
	else
	{
		...
		getQuotedStringOrNull($position).",".
		getQuotedStringOrNull($start_time).",'".
		$program_id."','".
		$program_item_id."')";

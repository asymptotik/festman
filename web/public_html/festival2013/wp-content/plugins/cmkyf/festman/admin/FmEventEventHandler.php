...
class FmEventEventHandler extends FmEventHandler
{
    ...
    function handleEvent(EventContext $context)
    {
        ...
        else if ($event_name == "remove_program_items")
        {
            $this->handleRemoveProgramItems($context);
        }
    ...
    function handleDeleteEvents(EventContext $context)
    {
        ...
        $action_message = "Deleted " . $count . " Event";
        if ($count == 0 || $count > 1)
    ...
    function handleRemoveProgramItem(EventContext $context)
    {
        if (isset($_SESSION['current_event']) == true)
        {
            ...
            $context->addActionMessage("Removed Program: " . $program->getName());
    ...
    // Fired from the event_editor.php page.
    //
    function handleRemoveProgramItems(EventContext $context)
    {
        $event = $this->storeCompleteEvent($context);
        $count = count(fmGetVar("program_program_item_ids"));
        $program = $event->getProgram();

        for ($i = 0; $i < $count; $i++)
        {
            $program_program_item_id = fmGetPost($_POST["program_program_item_ids"][$i]);
            $program->removeProgram_ProgramItemById($program_program_item_id);
            Program_ProgramItem::deleteProgram_ProgramItem($program_program_item_id);
        }

        $action_message = "Removed " . $count . " Program";
        if ($count == 0 || $count > 1)
            $action_message = $action_message . "s";
        $action_message = $action_message . ".";
